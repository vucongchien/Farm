...
//-----------------------------------inventory
    ...
    public void create() {
      ...
      //  plantManager.addPlant(new PlantRenderer(new Vector2(200, 100), PlantType.carrot));
        ...
        playerController = new PlayerController(new Vector2(200, 200), 150f, world,mapInteractionHandler, plantManager); // Use tile size
        playerImageManager = new PlayerImageManager();
        playerRenderer = new PlayerRenderer(playerController, playerImageManager, 64);

        playerCotrollerrNew=new PlayerCotrollerr(new Vector2(100,100),150f,world,mapInteractionHandler,plantManager);
    ...
    public void render() {
        ...
        camera.position.set(playerController.getPosition().x, playerController.getPosition().y, 0);
        camera.update();
